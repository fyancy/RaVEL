...
import numpy as np
from numba import njit, prange, int32, int64, float64, typeof


# @njit("Tuple((float64[:],int32[:],float64[:],int32[:],int32[:]))(int64,int64)")
# @njit(
#     (int64, int64, ProgressBarType),  # Signature for input arguments
# )
def generate_kernels(input_length, num_kernels):
    candidate_lengths = np.array((7, 9, 11), dtype=np.int32)
    # candidate_lengths = np.array([31, 63, 127], dtype=np.int32)
...
def generate_kernels_single_len(input_length, num_kernels):
...
def generate_kernels_original(input_length, num_kernels):
    candidate_lengths = np.array((7, 9, 11), dtype=np.int32)
    # candidate_lengths = np.array([31, 63, 127], dtype=np.int32)
    lengths = np.random.choice(candidate_lengths, num_kernels)
...
#       fastmath=True)
# @njit((float64[:, :], Tuple((float64[:], int32[:], float64[:], int32[:], int32[:])), ProgressBarType),
...
#       fastmath=True)
@njit(parallel=True, fastmath=True)
def apply_kernels_max(X, kernels):
    weights, lengths, biases, dilations, paddings = kernels
    ...
    for i in prange(num_examples):
        ...
        for j in range(num_kernels):
            ...
            _X[i, a2:b2] = \
                apply_kernel(X[i], weights[a1:b1], lengths[j], biases[j], dilations[j], paddings[j])[1]
            # only max is better than only ppv
