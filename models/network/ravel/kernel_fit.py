import numpy as np
from ..rocket.rocket_fun import generate_kernels_original
from .wavelet_fit import (fit_morlet_kernels, fit_laplace_kernels,
                          fit_hermitian_kernels, fit_harmonic_kernels)


def fit_kernels(X, num_kernels, multiscale=True, use_wavelet_kernel=True, multiwavelet=False, quantile_bias=True):
    if multiscale:
    ...
    if use_wavelet_kernel:
        ...
        weights, lengths, biases, dilations, paddings = generate_kernels_original(X.shape[1], num_kernels=num_kernels)
